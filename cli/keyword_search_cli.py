...
def main() -> None:
    ...
    tf_parser.add_argument("doc_id", type=int, help="Document ID for TF calculation")
    ...
    tfidf_parser.add_argument("doc_id", type=int, help="Document ID for TF-IDF calculation")
    ...
    match args.command:
        ...
        case "bm25tf":
            ...
            print(f"BM25 TF score of term '{args.term}' in document {args.doc_id}: {bm25tf:.2f}")
        case "bm25search":
            bm25_results = bm25_search(args.query)
            for idx, res in enumerate(bm25_results, start=1):
                print(f"{idx}. {res['id']} {res['title']} -- Score: {res['score']:.2f}")
        case _:
            parser.print_help()
